...
export function RevealCustomerPhoneCard({
  ...
  const onSend = async () => {
    ...
    try {
      ...
      setSent(true);
      setNotice(
        `Code sent to ${result.maskedHint}. Ask the customer to read it aloud.`,
      );
    } catch (e) {
  ...
  return (
    <section className="rounded-none border border-amber-200/80 bg-amber-50/40 p-4 shadow-none">
      <h2 className="text-sm font-semibold text-foreground">
        Reveal M-Pesa number
      </h2>
      <p className="mt-1 text-xs text-muted-foreground">
        Masked as {primary?.maskedHint ?? masked}
        {primary?.assignedMsisdn ? ` · assigned ${primary.assignedMsisdn}` : ""}
        . Fill the hidden digits, then send a code to the completed number.
      </p>
      {canReveal ? (
      ...
      {notice ? (
        <p className="mt-2 text-xs text-emerald-700">{notice}</p>
      ) : null}
    </section>
  );
}
