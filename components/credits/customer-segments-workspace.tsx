...
export function CustomerSegmentsWorkspace() {
  ...
  const exportCsv = () => {
    ...
    const slug =
      selectedItem?.sku?.trim() || selectedItem?.name?.trim() || "segment";
    anchor.href = url;
    ...
    <div className={DASHBOARD_MAX_WIDE}>
      ...
      >
        <Link
          ...
          className="inline-flex h-8 items-center gap-1.5 rounded-none border border-[color-mix(in_srgb,var(--order-ink,#15231f)_12%,transparent)] bg-white px-3 text-[12px] font-semibold tracking-[-0.02em] text-[color-mix(in_srgb,var(--order-ink,#15231f)_58%,transparent)] hover:text-[var(--order-ink,#15231f)]"
        >
      ...
      <div className={cn(CRM_WORKSPACE_SHELL, CRM_PANEL)}>
        <div
          className={cn(
            CRM_GRID,
            "xl:grid-cols-[minmax(16rem,18rem)_minmax(0,1fr)]",
          )}
        >
            ...
            <div className="space-y-4 p-4">
              <div>
                ...
                {productHits.length > 0 ? (
                  <ul className="mt-2 max-h-40 space-y-1 overflow-y-auto rounded-none border border-border/60 p-1.5">
                    {productHits.map((item) => (
                      <li key={item.id}>
                        <button
                          type="button"
                          className="w-full rounded-none px-2.5 py-2 text-left text-sm hover:bg-muted/50"
                          onClick={() => {
              ...
              {selectedItem ? (
                <div className="rounded-none border border-[#0f766e]/20 bg-[#ffffff]/60 px-3 py-2.5 dark:bg-[#0f766e]/10">
                    ...
                    <Link
                      href={
                        selectedIds.size === 0
                          ...
                          : `${APP_ROUTES.customerEmailCampaignNew}?customerIds=${Array.from(
                              selectedIds,
                            )
                              .slice(0, 500)
