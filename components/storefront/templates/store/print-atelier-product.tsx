...
/**
 * Print atelier PDP — sage CTAs, serif title, rounded gallery.
 * Craft bar: shop.3deastafrica.com product pages.
 */
export function PrintAtelierProduct({
  slug,
  item,
}: {
  slug: string;
  ...
  return (
    <div
      className={cn(styles.root, styles.body, printAtelierFontVariables, styles.pdp)}
        ...
        <div className={styles.pdpGrid}>
          ...
          <section className={styles.pdpInfo}>
            ...
            <div className={styles.pdpNote}>
              <h3 className={styles.pdpNoteTitle}>Add a note to your order</h3>
              <p className={styles.pdpNoteHint}>
                If your order is custom, add relevant detail
              </p>
              <textarea
                className={styles.pdpNoteInput}
                value={note}
                onChange={(e) => setNote(e.target.value.slice(0, 500))}
                placeholder="For ex. your name"
                rows={3}
                maxLength={500}
              />
