...
export function ButcherBoardTickets({ types }: { types: PublicCatalogType[] }) {
  ...
  return (
    <nav className={styles.tickets} aria-label="Shop by type">
      {visible.map((type) => {
        ...
        return (
          ...
          >
            {type.label}
            {type.itemCount != null ? (
              <span className={styles.ticketCount}>{type.itemCount}</span>
            ) : null}
          </Link>
        );
      })}
