...
export function BoutiqueShelfLabels({ types }: { types: PublicCatalogType[] }) {
  ...
  return (
    <nav className={styles.labels} aria-label="Shop by type">
      {visible.map((type) => {
        ...
        return (
          ...
          >
            {type.label}
            {type.itemCount != null ? (
              <span className={styles.plaqueCount}>{type.itemCount}</span>
            ) : null}
          </Link>
        );
      })}
