    ...
    <>
      ...
      >
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-4 gap-x-1 gap-y-0 px-2.5 py-2 sm:grid-cols-4 sm:gap-x-6 sm:gap-y-5 sm:px-6 sm:py-5">
          {ITEMS.map(({ icon: Icon, label, shortLabel, sub, ...rest }) => {
            const talkToUs = "talkToUs" in rest && rest.talkToUs;
            const content = (
              <>
                <span
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0 hidden text-[11px] text-white/65 sm:block">
                    {sub}
                  </p>
                </div>
              </>
            ...
            if (talkToUs) {
              return (
                <button
                  ...
                  className="flex flex-col items-center gap-1 text-center transition hover:opacity-90 sm:flex-row sm:items-center sm:gap-3 sm:text-left"
                >
                  {content}
                </button>
              );
        ...
        <div className="border-t border-white/8 py-1.5 text-center sm:py-3">
          <p className="mx-auto max-w-7xl px-3 text-[9px] leading-snug text-white/70 sm:px-4 sm:text-[11px]">
            ...
            <span className="hidden sm:inline">
              &copy; {year} {storeName}. Prices and availability may vary by
              branch.
            </span>
          </p>
        </div>
      ...
      <TalkToUsModal
        ...
        slug={slug}
        title="Talk to us"
        description={`Send a message to ${storeName}.`}
      />
    </>
  );
}
