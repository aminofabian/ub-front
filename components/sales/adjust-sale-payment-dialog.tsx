...
export function AdjustSalePaymentDialog({
  ...
  const hasRestricted =
    sale?.payments.some((p) => isRestricted(p.method)) ?? false;
  const canSave =
    !!sale &&
    !hasRestricted &&
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md gap-0 p-0 sm:max-w-lg">
            ...
            <div className="space-y-4">
              ...
              {hasRestricted ? (
                <p className="text-sm text-destructive">
                  This sale uses wallet or loyalty tender. Void and re-sell
              ...
              <div className="space-y-3">
                {drafts.map((draft, index) => (
                  <div
                    key={index}
                    className="space-y-3 rounded-none border border-[color-mix(in_srgb,var(--order-ink,#15231f)_12%,transparent)] p-3.5"
                  >
        ...
        <DialogFooter className="gap-2 border-t border-[color-mix(in_srgb,var(--order-ink,#15231f)_12%,transparent)] px-6 py-4 sm:gap-2">
          ...
          <Button
            type="button"
            className="bg-[#0f766e] text-white hover:bg-[#9A7A3F]"
            onClick={() => void onSave()}
            disabled={!canSave}
          >
