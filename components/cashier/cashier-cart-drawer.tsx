...
const fieldClass = (extra?: string) =>
  cn(
    "rounded-none border border-border/55 bg-background px-3 text-sm shadow-sm",
...
export type CashierCartDrawerProps = {
  ...
  onClearSale?: () => void;
  /** Opens the Open-shift flow when a sale is blocked by a closed till. */
  onOpenShift?: () => void;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        {saleComplete ? (
          <>
            <div className="shrink-0 border-b border-[color-mix(in_srgb,var(--pos-ink,#1c1915)_10%,transparent)] px-4 py-3 print:hidden dark:border-border/40">
          ...
          <>
            <div
              className="relative shrink-0 border-b border-[color-mix(in_srgb,var(--pos-ink,#1c1915)_10%,transparent)] px-3.5 py-2.5 dark:border-border/40"
              style={{
                background:
            ...
            >
              <DialogHeader className={cn("relative min-w-0", desktop && "pr-7")}>
                <div className="flex items-center gap-2">
                  ...
                  {lines.length > 0 ? (
                    <button
                      type="button"
                      className="inline-flex items-center gap-1 rounded-none px-1.5 py-0.5 text-[11px] font-medium text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground"
                  ...
                  {!desktop ? (
                    <DialogClose asChild>
                      <button
                        type="button"
                        className="flex size-9 shrink-0 items-center justify-center rounded-none text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
              ...
              <div className="relative mt-1 flex items-end justify-between gap-3">
                <div className="flex min-w-0 items-baseline gap-1.5">
                  <span className="pos-market-section-label text-[2.1rem] leading-none tabular-nums">
                ...
                {cashReady && changeDueAmount > 0 ? (
                  <p className="shrink-0 rounded-none bg-emerald-500/15 px-2 py-0.5 text-[10px] font-semibold text-emerald-800 dark:text-emerald-200">
                    {creditChangeToWallet
                      ? `+${cashChange} wallet`
                      : `Change ${cashChange}`}
                  </p>
                ) : cashReady ? (
                  <p className="inline-flex shrink-0 items-center gap-1 rounded-none bg-[color-mix(in_srgb,var(--pos-primary)_14%,transparent)] px-2 py-0.5 text-[10px] font-semibold text-[var(--pos-primary)]">
              ...
              {linesOpen && lines.length > 0 ? (
                <ul className="pos-scroll mt-2 max-h-48 space-y-0.5 overflow-y-auto overscroll-contain rounded-none border border-border/40 bg-background/80 p-1 sm:max-h-28">
                  {lines.map((line) => {
                    ...
                    return (
                      <li
                        key={line.key}
                        className="flex items-center gap-2 rounded-none px-2 py-1.5 text-[11px]"
                      >
                        ...
                        <button
                          type="button"
                          className="flex size-8 shrink-0 items-center justify-center rounded-none text-muted-foreground hover:bg-muted hover:text-destructive sm:size-6"
            ...
            <div className="pos-scroll min-h-0 flex-1 overflow-y-auto overscroll-contain px-3.5 py-2.5">
              <div className="space-y-2.5">
                <section className="space-y-2">
                  ...
                  !isStkTender(payMethod) ? (
                    <p className="rounded-none border border-sky-200 bg-sky-50 px-2.5 py-1.5 text-[11px] font-medium text-sky-900 dark:border-sky-900 dark:bg-sky-950/30 dark:text-sky-100">
                  ...
                  {showCustomerPicker && showCollapsedOptional ? (
                    <div className="space-y-1">
                      <button
                        ...
                        className="flex h-9 w-full items-center justify-between rounded-none border border-dashed border-border/70 bg-card/80 px-3 text-left text-[12px] font-medium text-muted-foreground hover:bg-muted/40 disabled:opacity-40"
                      >
                  ...
                  {showCustomerPicker && !showCollapsedOptional ? (
                    <div className="space-y-2 rounded-none border border-border/45 bg-card/80 p-2.5">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <Button
                          type="button"
                          variant="secondary"
                          className="h-9 shrink-0 rounded-none px-2.5 text-xs font-semibold"
                          disabled={
                            !online ||
                            customerSearchBusy ||
                      ...
                      !selectedCustomer &&
                      customerPhoneQuery.trim() ? (
                        <div className="space-y-2 rounded-none border border-[color-mix(in_srgb,var(--pos-primary)_25%,var(--border))] bg-[color-mix(in_srgb,var(--pos-primary)_6%,transparent)] p-2.5">
                          ...
                          {canManageCustomers ? (
                            <>
                              ...
                              <Button
                                type="button"
                                className="h-9 w-full rounded-none text-xs font-semibold"
                                disabled={
                                  !online ||
                                  customerRegisterBusy ||
                      ...
                      {selectedCustomer ? (
                        <div className="rounded-none bg-muted/40 px-2.5 py-1.5 text-[12px]">
                          <div className="flex items-center justify-between gap-2">
                  ...
                  {!splitPay && payMethod === "cash" ? (
                    <div className="space-y-2 rounded-none border border-border/45 bg-card/80 p-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          className="shrink-0 rounded-none border border-border/50 bg-background px-2.5 py-2 text-[11px] font-semibold text-[var(--pos-primary)] hover:bg-[color-mix(in_srgb,var(--pos-primary)_8%,transparent)]"
                          onClick={() => {
                            const next = payableTotal.toFixed(2);
                            lastAutoTenderRef.current = next;
                            setCashTenderStr(next);
                      ...
                      <div className="flex gap-1 overflow-x-auto pb-0.5">
                        {CASH_QUICK_AMOUNTS.map((n) => (
                          <button
                            ...
                            className="shrink-0 rounded-none border border-border/50 bg-background px-2 py-1 text-[11px] font-semibold tabular-nums transition hover:border-[var(--pos-primary)] hover:text-[var(--pos-primary)]"
                          >
                            {n}
                          </button>
                        ))}
                      ...
                      {cashReady && changeDueAmount > 0 && canLookupCustomers ? (
                        <label
                          className={cn(
                            "flex cursor-pointer items-center gap-2 rounded-none border px-2.5 py-2 text-[11px]",
                            creditChangeToWallet
                    ...
                    <div className="space-y-2 rounded-none border border-border/45 bg-card/80 p-2.5">
                      ...
                      stkPushStatus === "awaiting_till" ? (
                        <>
                          {stkPushStatus === "awaiting_till" &&
                          payMethod === "mpesa_manual" ? (
                            <p className="rounded-none border border-sky-200 bg-sky-50 px-3 py-2 text-[12px] font-medium text-sky-900 dark:border-sky-900 dark:bg-sky-950/30 dark:text-sky-100">
                        ...
                        <div className="space-y-2">
                          <p className="rounded-none border border-sky-200 bg-sky-50 px-3 py-3 text-center text-[12px] font-medium text-sky-900 dark:border-sky-900 dark:bg-sky-950/30 dark:text-sky-100">
                          ...
                          {onCancelInFlightMpesa ? (
                            <Button
                              type="button"
                              variant="ghost"
                              className="h-10 w-full rounded-none text-sm font-semibold text-muted-foreground"
                        ...
                        <div
                          ...
                          aria-live="polite"
                        >
                          <div className="flex gap-2 rounded-none border border-emerald-300 bg-emerald-50 px-3 py-2.5 dark:border-emerald-800 dark:bg-emerald-950/50">
                            <Check
                      ...
                      {canLookupCustomers ? (
                        <div className="space-y-1.5">
                          {!selectedCustomer ? (
                            ...
                            <p className="rounded-none bg-muted/50 px-2.5 py-1.5 text-[11px] text-muted-foreground">
                ...
                {lines.length === 0 ? (
                  <div className="flex flex-col items-center gap-2 rounded-none border border-dashed border-border/55 py-6 text-center">
                    <ShoppingBag
                ...
                {outboxCount > 0 ? (
                  <p className="rounded-none border border-amber-200/60 bg-amber-50/80 px-3 py-2 text-[12px] text-amber-950 dark:border-amber-900/40 dark:bg-amber-950/25 dark:text-amber-100">
                ...
                {error ? (
                  /open a shift/i.test(error) && onOpenShift ? (
                    <div className="flex items-center justify-between gap-2 rounded-none border border-destructive/35 bg-destructive/5 px-2.5 py-1.5 text-[11px] text-destructive">
            ...
            <div className="shrink-0 border-t border-[color-mix(in_srgb,var(--pos-ink,#1c1915)_10%,transparent)] bg-background/95 px-3.5 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-md dark:border-border/40">
              {!branchSelected ? (
                <p className="mb-1.5 rounded-none border border-amber-200/50 bg-amber-50/90 px-2.5 py-1.5 text-[11px] text-amber-950 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-100">
                  Pick a branch in the top nav to check out.
                </p>
              ) : null}
              {completeBlockedHint ? (
                <p className="mb-1.5 rounded-none bg-muted/40 px-2.5 py-1.5 text-[11px] text-muted-foreground">
                  {completeBlockedHint}
                </p>
              ) : null}
              <div className="flex items-stretch gap-1.5">
                ...
                <Button
                  type="button"
                  className={cn(
                    "h-12 min-w-0 flex-1 rounded-none text-sm font-bold tracking-tight shadow-md transition-all duration-200 sm:rounded-none",
