  ...
  return (
    ...
    >
      ...
      <div className="relative grid gap-0 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        <div className="flex flex-col justify-center gap-1 border-b border-[#E6E1D8] px-3 py-2.5 lg:border-b-0 lg:border-r">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
            ...
            <p className={cn("text-[9px] uppercase tracking-[0.1em]", HUB_MUTED)}>
              {revenueLabel}
            </p>
            {live ? (
              <span className="ml-auto inline-flex items-center gap-1 text-[9px] font-semibold uppercase tracking-wide text-emerald-700">
                <span
                  className={cn(
          ...
          <div className="flex flex-wrap items-end gap-2">
            <p
              ...
              className={cn(
                "text-[clamp(1.45rem,2.8vw,1.85rem)] font-medium leading-none tracking-tight text-[#141414] tabular-nums",
            ...
            {trend ? (
              <span
                className={cn(
                  "mb-0.5 inline-flex items-center border px-1 py-0.5 text-[9px] font-semibold tabular-nums",
                  trendTone === "positive" &&
          ...
          <p className="line-clamp-1 text-[11px] leading-snug text-[#5A5A5A]">
            {headline}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 bg-[#E6E1D8] sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
          {metrics.map((metric) => {
            const body = (
              <div className="bg-white px-2.5 py-2 transition-colors hover:bg-[#FCFAF6]">
                <p className={cn("text-[9px] font-medium uppercase tracking-[0.08em]", HUB_MUTED)}>
                  {metric.label}
                </p>
                <p className="mt-0.5 text-sm font-semibold tracking-tight text-[#141414] tabular-nums">
                  {metric.value}
                </p>
                {metric.hint ? (
                  <p
