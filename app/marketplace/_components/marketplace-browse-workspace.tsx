...
} from "./marketplace-browse-ledger";
import { MarketplaceOrderWorkspace } from "./marketplace-order-panel";
...
import { InheritSupplierDialog } from "./inherit-supplier-dialog";

const SEARCH_DEBOUNCE_MS = 320;
const PRODUCT_PAGE_SIZE = 100;
const SUPPLIER_PAGE_SIZE = 100;

export type MarketplaceBrowseVariant = "public" | "tenant";

export function MarketplaceBrowseWorkspace({
  variant = "public",
  canInherit = false,
}: {
  variant?: MarketplaceBrowseVariant;
  /** When true (tenant + permission), show inherit-shelf actions. */
  canInherit?: boolean;
}) {
  return (
    <Suspense
      fallback={
        <div
          className={cn(
            "flex items-center justify-center bg-[radial-gradient(120%_80%_at_50%_-10%,color-mix(in_srgb,#0f766e_12%,#f7f4ef),#efeae2_42%,#e7e1d6)] text-sm text-muted-foreground",
            variant === "tenant" ? "h-full min-h-[20rem]" : "min-h-dvh",
          )}
    ...
    >
      <MarketplaceBrowseWorkspaceInner
        variant={variant}
...
function MarketplaceBrowseWorkspaceInner({
  variant,
  canInherit,
}: {
  variant: MarketplaceBrowseVariant;
  ...
  return (
    <div
      className={cn(
        "bg-[radial-gradient(120%_80%_at_50%_-10%,color-mix(in_srgb,var(--pos-primary,#0f766e)_12%,#f7f4ef),#efeae2_42%,#e7e1d6)] text-[var(--pos-ink,#1c1915)]",
        isTenant ? "flex h-full min-h-0 flex-col overflow-hidden" : "min-h-dvh",
      )}
      style={{ ["--pos-primary" as string]: "#0f766e" }}
    >
      {!isTenant ? (
      ...
      <main
        className={cn(
          ...
          isTenant
            ? "h-full min-h-0 px-2 pt-1 sm:px-4 sm:pt-1.5"
            : cn(
                "px-2 pt-2 sm:px-5 sm:pt-3",
      ...
      >
        ...
        >
          <div className="min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">
              {isTenant ? "Buying · Find suppliers" : "Browse"}
            </p>
            <h1 className="mt-0.5 font-heading text-xl font-semibold tracking-[-0.03em] text-[var(--pos-ink,#1c1915)] sm:text-2xl">
              {isTenant ? "Inherit a supplier shelf" : "Supplier marketplace"}
            </h1>
            <p className="mt-0.5 truncate font-mono text-[11px] text-muted-foreground">
              {isTenant
                ? "discover → inherit → order"
                : `area → supplier → ${isLedger ? "ledger" : "shelf"}`}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {isTenant && canInherit && ordering && supplierDetail ? (
              <button
                type="button"
                className="inline-flex h-8 items-center gap-1.5 bg-[var(--pos-primary,#0f766e)] px-3 text-[10px] font-semibold uppercase tracking-[0.1em] text-white hover:bg-[color-mix(in_srgb,#0f766e_88%,#000)]"
        ...
        <div
          className={cn(
            ...
            isTenant
              ? "h-full min-h-0 max-lg:rounded-none max-lg:border-x-0 max-lg:shadow-none"
              : cn(
                  ...
                  <p className="mt-0.5 text-[11px] text-muted-foreground">
                    {loading || detailLoading
                      ...
                      : tab === "products" && activeSupplierId && supplierDetail
                        ? `${supplierDetail.products.length} product${supplierDetail.products.length === 1 ? "" : "s"}${isTenant && canInherit ? " · inherit or order" : " · tap to order"}`
                        : tab === "products"
                  ...
                  <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
                    {detailLoading ? (
                      ...
                      <MarketplaceOrderWorkspace
                        ...
                        canInherit={canInherit}
                        onInherit={
                          canInherit ? () => setInheritOpen(true) : undefined
                        }
                      />
                    ) : (
