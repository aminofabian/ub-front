...
type Props = {
  ...
  m: Pick<ProductMutationsApi,
    ...
    "onCreateParent" | "pendingCreateImage" | "setPendingCreateImage"
  >;
...
function InputRow({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cn("grid gap-3 sm:grid-cols-2", className)}>{children}</div>;
}

function Label({ children, className }: { children: React.ReactNode; className?: string }) {
  return <label className={cn("flex flex-col gap-1.5 text-xs font-medium text-muted-foreground", className)}>{children}</label>;
}
...
export function ProductCreateDrawer({ open, onClose, banner, catalog, m, canLinkSupplier, canListSuppliers, currencyCode }: Props) {
  ...
  return (
    <FormDrawer
      ...
      description={isGroup ? "Create a label that holds variants." : "Add a sellable product to the catalog."}
      contextLabel="Catalog"
      icon={<PackagePlus className="size-5 text-primary" aria-hidden />}
      footer={
        <div className="flex flex-wrap justify-end gap-2">
          <Button type="button" variant="outline" onClick={onClose}>Cancel</Button>
          <Button type="submit" form="create-parent-form" disabled={catalog.itemTypes.length === 0}>
    ...
    >
      <form id="create-parent-form" className="space-y-5" onSubmit={m.onCreateParent}>
        ...
        {/* ── Product Structure ── */}
        <div>
          <SectionHeader icon={Boxes} label="Product structure" hint="Choose how this product is organised" />
          <div className="grid gap-2 sm:grid-cols-2">
            <button
              type="button"
              onClick={() => m.setParentDraft(p => ({ ...p, productStructure: "standalone", isSellable: true }))}
              className={cn(
            ...
            >
              <Package className={cn("mt-0.5 size-5 shrink-0", !isGroup ? "text-primary" : "text-muted-foreground")} aria-hidden />
              <div>
                <p className={cn("text-sm font-medium", !isGroup ? "text-primary" : "text-foreground")}>Standalone SKU</p>
                <p className="text-[11px] text-muted-foreground">Has its own barcode, price and stock. Sold individually.</p>
              </div>
            </button>
            <button
              type="button"
              onClick={() => m.setParentDraft(p => ({ ...p, productStructure: "group", isSellable: false }))}
              className={cn(
                "flex items-start gap-3 rounded-xl border p-3 text-left transition",
                isGroup
                  ? "border-primary bg-primary/[0.04] ring-1 ring-primary/20"
            ...
            >
              <Layers className={cn("mt-0.5 size-5 shrink-0", isGroup ? "text-primary" : "text-muted-foreground")} aria-hidden />
              <div>
                <p className={cn("text-sm font-medium", isGroup ? "text-primary" : "text-foreground")}>Product group</p>
                <p className="text-[11px] text-muted-foreground">Just a label. Variants inside carry SKUs, prices and stock.</p>
              </div>
            </button>
          </div>
        </div>
        ...
        {/* ── Identity ── */}
        <div>
          <SectionHeader icon={Tag} label="Identity" hint={isGroup ? "Name and classification" : "Names, codes and classification"} />
            ...
            <Label>
              ...
              >
                {catalog.itemTypes.map(t => (
                  <option key={t.id} value={t.id}>{t.label} ({t.key})</option>
                ))}
              </select>
            </Label>
            <Label>
              ...
              <input
                className={icClass()}
                placeholder={isGroup ? "e.g. Brookside Milk" : "Customer-facing title"}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {m.nextAutoSkuHint && (
                      <Button
                        ...
                        onClick={() => m.setParentDraft(p => ({ ...p, sku: m.nextAutoSkuHint! }))}
                      >
        ...
        {/* ── Photo ── */}
        <div>
          <SectionHeader icon={Camera} label="Photo" hint="Upload a cover image" />
          {m.pendingCreateImage ? (
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-[11px] text-muted-foreground">or click to browse — JPG, PNG, WEBP</p>
              </div>
            </div>
          )}
          <input
        ...
        {/* ── Categorisation ── */}
        <div>
          <SectionHeader icon={Boxes} label="Categorisation" hint="Category, brand and size" />
          <div className="space-y-3">
            <Label>
              Category
              <select
        ...
        {!isGroup && (
          <div>
            <SectionHeader icon={Tag} label="Pricing" hint="Buy cost and sell price" />
            <div className="space-y-3">
              <InputRow className="sm:grid-cols-3">
                <Label>
                  ...
                  <input
                    ...
                    onChange={(e) => m.setParentDraft(p => ({ ...p, buyingPrice: e.target.value }))}
                  />
                </Label>
                <Label>
                  Pack qty
                  <input
                    ...
                    onChange={(e) => m.setParentDraft(p => ({ ...p, bundleQty: e.target.value }))}
                  />
                </Label>
                <Label>
                  ...
                  <input
                    ...
                    onChange={(e) => m.setParentDraft(p => ({ ...p, bundlePrice: e.target.value }))}
                  />
                </Label>
              </InputRow>
              <Label>
        ...
        {!isGroup && (
          <div>
            <SectionHeader icon={Tag} label="Stock & tracking" hint="Inventory levels and behaviour" />
            <div className="space-y-3">
              <InputRow className="sm:grid-cols-3">
                <Label>
                  Min stock
                  <input
                    ...
                    onChange={(e) => m.setParentDraft(p => ({ ...p, minStockLevel: e.target.value }))}
                  />
                </Label>
                <Label>
                  Reorder at
                  <input
                    ...
                    onChange={(e) => m.setParentDraft(p => ({ ...p, reorderLevel: e.target.value }))}
                  />
                </Label>
                <Label>
                  Reorder qty
                  <input
                    ...
                    onChange={(e) => m.setParentDraft(p => ({ ...p, reorderQty: e.target.value }))}
                  />
                </Label>
              </InputRow>
              <div className="flex flex-wrap items-center gap-5 rounded-lg border border-border/40 bg-muted/20 px-3 py-2.5">
                <label className="flex items-center gap-2 text-xs">
                  <input
                    ...
                    onChange={(e) => m.setParentDraft(p => ({ ...p, isStocked: e.target.checked }))}
                ...
                <label className="flex items-center gap-2 text-xs">
                  <input
                    ...
                    onChange={(e) => m.setParentDraft(p => ({ ...p, isSellable: e.target.checked }))}
                    className="size-4 rounded border-border text-primary"
                  />
                  <span>Sellable</span>
                </label>
                <label className="flex items-center gap-2 text-xs">
                  <input
                    ...
                    onChange={(e) => m.setParentDraft(p => ({ ...p, isWeighed: e.target.checked }))}
                    className="size-4 rounded border-border text-primary"
                  />
        ...
        {/* ── Details ── */}
        <div>
          <SectionHeader icon={FileText} label="Details" hint="Extra product info" />
          <div className="space-y-3">
            <Label>
              Description
              <textarea
        ...
        {!isGroup && canLinkSupplier && (
          <div>
            <SectionHeader icon={Tag} label="Supplier" hint="Optional supplier link" />
            <div className="space-y-3">
              {canListSuppliers && (
                <Button
                  type="button"
              ...
              <InputRow>
                ...
                <Label>
                  ...
                  <input
                    ...
                    onChange={(e) => m.setParentDraft(p => ({ ...p, defaultCostPrice: e.target.value }))}
                  />
                </Label>
              </InputRow>
              <label className="flex items-center gap-2 text-xs">
                <input
                  ...
                  onChange={(e) => m.setParentDraft(p => ({ ...p, setPrimarySupplier: e.target.checked }))}
                  className="size-4 rounded border-border text-primary"
                />
