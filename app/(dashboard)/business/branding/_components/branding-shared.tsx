...
export function formFromBranding(b: BrandingRecord | undefined | null): FormState {
  return {
...
export function buildPatch(next: FormState): BrandingPatchPayload {
  return {
...
export function keepUnsavedFields(
  prev: FormState,
...
export function isFormDirty(form: FormState, saved: FormState): boolean {
  return (
...
export function StartOpenDetails({
  startOpen,
  children,
}: {
  startOpen: boolean;
...
export function BrandingColorPresetPicker({
  primaryColor,
  accentColor,
  onSelect,
}: {
...
export function BrandingIdentityFields({
  ...
  return (
    <div id="branding-identity" className="space-y-4 scroll-mt-24">
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <input
            ...
            placeholder={businessName ?? "Your storefront name"}
          />
          <p className={hintClass()}>
      ...
      <div className="grid grid-cols-2 gap-2">
        <span className="flex min-h-[5rem] items-center justify-center border border-[color-mix(in_srgb,var(--order-ink,#15231f)_12%,transparent)] bg-white p-2">
          <TenantLogo
            brand="Your logo"
            logoUrl={logoUrl}
            primaryColor={form.primaryColor}
        ...
        <span className="flex min-h-[5rem] items-center justify-center border border-neutral-800 bg-[#0f172a] p-2">
          <TenantLogo
            brand="Your logo"
            logoUrl={logoDarkUrl}
            primaryColor={form.primaryColor}
      ...
      <p className={hintClass()}>
        Light and dark marks upload in the Logos section — they go live
        immediately.
      </p>
    </div>
  );
...
export function BrandingSearchFields({
  form,
  seoDisplayName,
  seoLocation,
  onFormChange,
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <input
          ...
          placeholder={defaultStorefrontMetaTitle(seoDisplayName, seoLocation)}
        />
        <p className={hintClass()}>
      ...
      <div className="space-y-2">
        <div className="flex items-baseline justify-between gap-3">
          <label className={labelClass()} htmlFor="branding-meta-description">
            Google snippet
          </label>
          <span className="text-xs tabular-nums text-muted-foreground">
