    ...
    return (
      <WhiteCard className="px-5 py-12">
        <p className="max-w-[65ch] text-[15px] leading-relaxed" style={{ color: INK }}>
          Pick a walk stop from the left to see what lives on that shelf — stock
          levels, fast movers, and every product tagged to the zone.
        </p>
      </WhiteCard>
    );
  }
    ...
    <div className="min-h-0 space-y-3 overflow-y-auto">
      <WhiteCard className="px-4 py-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h2
              className="mt-1 text-[1.5rem] font-bold leading-tight tracking-[-0.03em]"
              style={{ color: INK }}
            >
              {aisle.name}
            </h2>
            <p className="mt-1 text-[12px]" style={{ color: MUTED }}>
              Walk stop {walkIndex + 1} of {walkStops}
              {!aisle.active ? " · inactive" : ""}
            </p>
          </div>
          <Button type="button" className="h-10 rounded-none" asChild>
            <Link
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between text-[11px] font-semibold uppercase tracking-[-0.02em]" style={{ color: MUTED }}>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatBlock
          ...
          hint={branchId ? "At this branch" : "Select a branch for stock"}
          lead
        />
        <StatBlock
          label="Low stock"
      ...
      {bestSellers.length > 0 ? (
        <WhiteCard className="px-4 py-4">
          ...
          <p className="mt-1 text-[12px]" style={{ color: MUTED }}>
            Top movers in this zone from recent till sales
          </p>
              ...
              return (
                <li key={item.id}>
                  ...
                  >
                    ...
                    <span className="shrink-0 tabular-nums text-[12px]" style={{ color: MUTED }}>
      ...
      <WhiteCard className="overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[12px]" style={{ color: MUTED }}>
                ...
                {hasMore ? "+" : ""} · sorted by quantity
              </p>
            </div>
          </div>
          {(stats?.zeroStock ?? 0) > 0 ? (
            <span className="inline-flex items-center gap-1 bg-[#fff4e5] px-2 py-1 text-[11px] font-medium text-[#8a5a00]">
              <AlertTriangle className="size-3.5" aria-hidden />
              {stats?.zeroStock} empty
            </span>
          ) : null}
        </div>
        ...
        {products.length === 0 ? (
          <p className="px-4 py-10 text-center text-[14px]" style={{ color: MUTED }}>
            No products tagged to this zone yet.
          </p>
        ) : (
          <ul className="divide-y divide-[#eef1f4]">
            {sortedByStock.slice(0, 40).map((row) => {
              ...
              const price = row.bundlePrice != null ? boardMoney(row.bundlePrice, currency) : null;
              return (
