...
export default function PricingPage() {
  ...
  const [suggestion, setSuggestion] =
    useState<SellPriceSuggestionRecord | null>(null);
  ...
  const [lastSellingPrice, setLastSellingPrice] =
    useState<SellingPriceResponseRecord | null>(null);
  ...
  const onSuggest = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const row = await fetchSellPriceSuggestion(
        id,
        suggestSupplierId.trim() || undefined,
      );
      setSuggestion(row);
  ...
  const onCreateRule = useCallback(async () => {
    ...
    try {
      const paramsJson = JSON.stringify({
        marginPercent: newRuleMargin.trim(),
      });
      await postPriceRule({
        name,
  ...
  const onUpdateRule = useCallback(async () => {
    ...
    try {
      const paramsJson = JSON.stringify({
        marginPercent: editRuleMargin.trim(),
      });
  ...
  if (!allowed) {
    return (
      <DashboardAccessDenied
        ...
        description={
          <>
            You need <code className="text-xs">{Permission.PricingRead}</code>,{" "}
            <code className="text-xs">{Permission.PricingSellPriceSet}</code>,
            or <code className="text-xs">{Permission.PricingRulesManage}</code>.
          </>
        }
  ...
  return (
    <div className={DASHBOARD_MAX_WIDE}>
      <DashboardPageHero
        ...
        description={
          <>
            Suggested sell price uses the latest landed cost and active
            margin rules. Selling prices are effective-dated rows (
            {currency}). Use the actions on the right to open forms in a
            drawer.
          </>
        }
      >
      ...
      />
        ...
        {canRead ? (
          <div className="space-y-4 rounded-none border bg-white p-3">
            <h3 className="text-sm font-medium">Reference</h3>
            <p className="text-xs text-muted-foreground">
              Margin rules and tax rates for this business.{" "}
              <span className="font-medium text-foreground">Suggest price</span>{" "}
              in the header runs a cost-to-sell lookup for an item.
            </p>

            <div className="space-y-2">
              <h4 className="text-xs font-medium tracking-[-0.02em] text-muted-foreground">
                Price rules
              </h4>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Active rates only (for reference).
              </p>
              <div className="overflow-x-auto rounded-none border">
        ...
        {canRead ? (
            ...
            footer={
              <div className="flex flex-wrap justify-end gap-2">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => setSuggestDrawerOpen(false)}
                >
                  Close
                </Button>
                <Button
          ...
          >
            <FormDrawerFields
              legend="Inputs"
              hint="Paste item UUID from catalog; supplier UUID is optional."
            >
                ...
                <dl className="grid gap-2 text-sm sm:grid-cols-2">
                  <div className="flex justify-between gap-2">
                    ...
                    <dd className="tabular-nums">
                      {num(suggestion.latestUnitCost)}
                    </dd>
                  </div>
                  <div className="flex justify-between gap-2">
                    <dt className="text-muted-foreground">Margin %</dt>
                    <dd className="tabular-nums">
                      {num(suggestion.marginPercent)}
                    </dd>
                  </div>
        ...
        {canManageRules ? (
              ...
              <>
                {!canRead ? (
                  <span>
                    ...
                    . You can still create records here.
                  </span>
                ) : (
                  <span>
                    Rule type is fixed to{" "}
                    <code className="rounded bg-muted px-0.5 text-xs">
                      MARGIN_PERCENT
                    </code>{" "}
                    (JSON param{" "}
          ...
          >
            ...
            <FormDrawerFields
              ...
              hint={
                !canRead
                  ? "Read permission is required to list rules in this drawer."
                  : undefined
              }
            >
              {!canRead || rules.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  {canRead
                    ...
                    : "Requires read access to pick a rule."}
                </p>
              ) : (
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (rule) {
                          ...
                          setEditRuleMargin(
                            marginFromParamsJson(rule.paramsJson),
                          );
            ...
            <FormDrawerFields
              legend="New tax rate"
              hint="Rate is a non-negative percent. Inclusive affects how tax is presented on receipts."
            >
              <div className="flex flex-wrap items-end gap-3">
                <label className="flex min-w-[10rem] flex-1 flex-col gap-1 text-sm">
                  <span className="text-muted-foreground">Name</span>
        ...
        {canSetSell ? (
            ...
            footer={
              <div className="flex flex-wrap justify-end gap-2">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => setSellDrawerOpen(false)}
                >
                  Cancel
                </Button>
                <Button
                  type="submit"
                  form="pricing-sell-form"
                  disabled={loading}
                >
                  Save selling price
                </Button>
          ...
          >
            ...
            {lastSellingPrice ? (
              <p className="text-xs text-muted-foreground">
                Last saved row {lastSellingPrice.id}:{" "}
                {String(lastSellingPrice.price)} from{" "}
                {lastSellingPrice.effectiveFrom}
                {lastSellingPrice.effectiveTo
                  ? ` to ${lastSellingPrice.effectiveTo}`
                  : ""}
                .
              </p>
            ) : null}
