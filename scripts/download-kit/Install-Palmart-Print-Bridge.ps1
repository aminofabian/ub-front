...
# Copies into %LOCALAPPDATA%, starts it hidden, and auto-starts at every logon
# (Startup folder + Scheduled Task). No console window required after install.
...
function Stop-BridgeOnPort {
  # Best-effort: free port 19500 if a previous bridge is stuck.
  try {
...
function Write-HiddenVbs([string]$NodeExe, [string]$BridgeFile, [string]$WorkDir, [string]$OutVbs, [string]$OutLog) {
  # VBS doubles quotes inside string literals.
...
sh.Run """$nodeEsc"" ""$bridgeEsc""", 0, False
"@
  Set-Content -Path $OutVbs -Value $content.TrimStart() -Encoding ASCII
}
...
# Uninstall helper (same install dir).
@(
